...
def __lldb_init_module(debugger, internal_dict):
    ...
    'command script add -f cclet.handle_command cclet')
...
def handle_command(debugger, command, result, internal_dict):
    ...
    old_value = frame.EvaluateExpression(old_member_value_expression, objc_options)
    change_value_expression = 'poco {0} = {1}'.format(old_string.name, new_value)
    ...
    change_len_expression = 'poco {0} = *(int*){1}'.format(current_member_len_info.name, new_value_length)
